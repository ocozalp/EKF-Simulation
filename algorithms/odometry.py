...
class Odometry:
    ...
    def sample(self, number_of_samples, use_communication, sensor=None):
        example_paths = dict()
        sense_lines = dict()
        ...
        for i in xrange(1, max_t_index):
            ...
            if use_communication:
                self.sense_robots(last_positions, number_of_samples)

        return result_distributions, example_paths, sense_lines

    def sense_robots(self, last_positions, number_of_samples):
        for i in last_positions:
            for j in last_positions:
                if j > i:
                    ...
                    if math.sqrt((p1[0] - p2[0]) ** 2 + (p1[1] - p2[1]) ** 2) <= 1:
